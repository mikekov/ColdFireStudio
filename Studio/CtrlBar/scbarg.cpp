...
CSizingControlBarG::CSizingControlBarG()
{
	...
	{
		gripper_height_ = ncm.iSmCaptionHeight;
...
bHorz=false;
    if (bHorz)
        rc.DeflateRect(gripper_height_, 0, 0, 0);
    else
        rc.DeflateRect(0, gripper_height_, 0, 0);

    // set position for the "x" (hide bar) button
    CPoint ptOrgBtn;
    if (bHorz)
        ptOrgBtn = CPoint(rc.left - gripper_height_, rc.top);
    else
        ptOrgBtn = CPoint(rc.right - gripper_height_, rc.top - gripper_height_);
...
void CSizingControlBarG::NcPaintGripper(CDC* pDC, CRect rcClient)
{
    ...
    if (bHorz)
    {   // gripper at left
        gripper.left -= gripper_height_;
        gripper.right = gripper.left + 3;
        gripper.top = rcbtn.bottom + 3;
    }
    else
    {   // gripper at top
        gripper.top -= gripper_height_;
...
LRESULT CSizingControlBarG::OnNcHitTest(CPoint point)
{
    ...
    auto nRet = baseCSizingControlBarG::OnNcHitTest(point);
    if (nRet != HTCLIENT)
        return nRet;
...
void CSCBButton::Paint(CDC* pDC)
{
	...
	pDC->DrawText(L"r", 1, rc, DT_CENTER | DT_VCENTER | DT_SINGLELINE);	// x-like
