...
int CSizingControlBar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if (CDockBar* p= dynamic_cast<CDockBar*>(GetParent()))
		p->SetBorders();
...
CSize CSizingControlBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
	...
	layout.hDWP = pDockBar->m_bLayoutQuery ? NULL : ::BeginDeferWindowPos(static_cast<int>(arrSCBars.GetSize()));
	...
	int nLengthTotal = bHorz ? rc.Width() + 0 : rc.Height() - 0;
...
void CSizingControlBar::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS* ncsp)
{
	...
	if (!IsFloating() && m_pDockBar != NULL)
	{
		...
		if (nThis < arrSCBars.GetUpperBound())
			m_dwSCBStyle |= bHorz ? SCBS_EDGERIGHT : 0 /*SCBS_EDGEBOTTOM*/; // no useless bottom edge
	}

	NcCalcClient(&ncsp->rgrc[0], m_nDockBarID);
}

void CSizingControlBar::NcCalcClient(RECT* rect, UINT nDockBarID)
{
	CRect rc(rect);
...
void CSizingControlBar::OnNcPaint()
{
	...
	if (m_dwSCBStyle & SCBS_SHOWEDGES)
	{
		CRect edge; // paint the sizing edges
		for (int i = 0; i < 4; i++)
			if (GetEdgeRect(rcBar, GetEdgeHTCode(i), edge))
				DrawEdge(mdc, edge, GetEdgeHTCode(i));
	}
...
void CSizingControlBar::DrawEdge(CDC& dc, CRect edge, UINT edge_id)
{
	auto e= m_dwSCBStyle & SCBS_EDGEALL;
	if (e == 0 || edge.Width() <= 1 || edge.Height() <= 1)
		return;

	COLORREF light= ::GetSysColor(COLOR_3DHILIGHT);
	COLORREF gray= ::GetSysColor(COLOR_3DFACE);
	COLORREF dark= CalcColor(gray, ::GetSysColor(COLOR_3DSHADOW), 0.5f);

	light=gray;
	if ((e & SCBS_EDGELEFT) && edge_id == HTLEFT)
	{
		dc.FillSolidRect(edge.left, edge.top, 1, edge.Height(), dark);
	}
	else if ((e & SCBS_EDGERIGHT) && edge_id == HTRIGHT)
	{
		dc.FillSolidRect(edge.right - 1, edge.top, 1, edge.Height(), dark);
	}

	if (e & (SCBS_EDGETOP | SCBS_EDGEBOTTOM))
	{
		int a= edge.Width();

		if ((e & SCBS_EDGETOP) && edge_id == HTTOP)
			dc.FillSolidRect(edge.left, edge.top, edge.Width(), 1, dark);
	}
}
...
bool CSizingControlBar::GetEdgeRect(CRect rcWnd, UINT nHitTest, CRect& rcEdge)
{
	...
	case HTTOP:
		...
		rcEdge.DeflateRect(bHorz ? 0 : 1, 0);
		break;
	case HTRIGHT:
...
bool CSizingControlBar::NegotiateSpace(int nLengthTotal, bool bHorz)
{
	...
	for (int i = nFirst; i <= nLast; i++)
	{
		...
		if (bIsSizingBar)
			nLengthBar = bHorz ? pBar->m_szMinHorz.cx - 0 : pBar->m_szMinVert.cy - 0;
		else
		{
			CRect rcBar;
			pBar->GetWindowRect(&rcBar);
			nLengthBar = bHorz ? rcBar.Width() - 0 : rcBar.Height() - 0;
		}
		...
		if (bIsSizingBar)
		{
			nLengthActual += bHorz ? pBar->m_szHorz.cx - 0 : pBar->m_szVert.cy - 0;
...
void CSizingControlBar::AlignControlBars()
{
	...
	for (int i = nFirst; i <= nLast; i++)
	{
		...
		{
			if (!bHorz)
				rc.OffsetRect(0, nAlign - rc.top - 0);
		...
		nAlign += (bHorz ? rc.Width() : rc.Height()) - 0;
	}
...
void CSCBMiniDockFrameWnd::OnSize(UINT nType, int cx, int cy) 
{
	...
	if ((pBar != NULL) && (GetStyle() & MFS_4THICKFRAME) == 0
		...
		pBar->m_szFloat = CSize(cx + 0, cy + 0);
...
void CSCBMiniDockFrameWnd::OnGetMinMaxInfo(MINMAXINFO FAR* lpMMI)
{
	...
	if (pBar != NULL)
	{
		CRect r(CPoint(0, 0), pBar->m_szMinFloat - CSize(0, 0));
#ifndef _SCB_MINIFRAME_CAPTION
