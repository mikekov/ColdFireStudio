...
//	COLORREF light= ::GetSysColor(COLOR_3DHIGHLIGHT);
	...
	return CalcColor(dark, gray, 0.5f);
}
...
void CCustomTabCtrlItem::Draw(CDC& dc, CFont& font, bool selected, bool on_top, bool RTL, int selection_tinge)
{
	...
	case TAB_SHAPE5:
		{
			...
			COLORREF gray= selected_ ? ::GetSysColor(COLOR_WINDOW) : ::GetSysColor(COLOR_3DFACE);
			...
			{
				float r= float(i) / h;
				COLORREF c=gray;// CalcColor(gray, dark, 1.0f - r * r);
				if (i == 0 && !selected_)	// highlight at the top
					c = gray;//CalcColor(::GetSysColor(COLOR_3DHIGHLIGHT), ::GetSysColor(COLOR_3DFACE), 0.4f);
...
LOGFONT CustomTabCtrl::lf_default = {-11, 0, 0, 0, FW_NORMAL, 0, 0, 0,
...
{
	RegisterWindowClass();
	auto lf = GetDefaultFont();
	lf.lfHeight = ToLogicalPixels(CSize(0, lf.lfHeight)).cy;
	SetControlFont(lf);
...
BEGIN_MESSAGE_MAP(CustomTabCtrl, CControlBar)
...
{
	return !!CControlBar::Create(CustomTabCtrl_CLASSNAME, _T(""), dwStyle, rect, parent_wnd, id);
}
...
void CustomTabCtrl::DrawCtrl(CDC& dc, int selection_tinge)
{
	...
	// clear background
	mem_dc.FillSolidRect(client, GetSysColor(COLOR_BTNFACE));
	...
	const auto count= a_items_.GetSize();
	for (auto i= 0; i < count; ++i)
		if (a_items_[i]->shape_ && !a_items_[i]->selected_)
			a_items_[i]->Draw(mem_dc, font_, a_items_[i]->highlighted_, !!(GetStyle() & CTCS_TOP), RTL, 0);

	// draw selected tab item
	if (item_selected_ != -1 && a_items_[item_selected_]->shape_)
		a_items_[item_selected_]->Draw(mem_dc, font_selected_, true, !!(GetStyle() & CTCS_TOP), RTL, selection_tinge);
...
//	mem_dc.FillSolidRect(client.left, client.bottom - 3, client.Width(), 10, 0);//CalcColor(GetSysColor(COLOR_BTNFACE), 0, 0.5f));
...
void CustomTabCtrl::OnMouseMove(UINT flags, CPoint point)
{
	...
	if (button_id_down_>=0 && item_drag_dest_>=0)
	{
		...
		if (item_drag_dest_>=a_items_.GetSize())
			item_drag_dest_ = static_cast<int>(a_items_.GetSize()-1);
		...
		if (x>=cl.right)
		{
			item_drag_dest_++;
			if (item_drag_dest_>=a_items_.GetSize())
				RecalcLayout(RECALC_NEXT_PRESSED, static_cast<int>(a_items_.GetSize()-1));
			else
		...
		else if (x>=x2)
		{
			item_drag_dest_++;
			if (item_drag_dest_>=a_items_.GetSize())
				RecalcLayout(RECALC_ITEM_SELECTED, static_cast<int>(a_items_.GetSize()-1));
			else
...
{
	if (index == -1)
		index = static_cast<int>(a_items_.GetSize());
	...
	{
		for (int i= static_cast<int>(a_items_.GetSize()-1); i > index; i--)
		{
			CString s;
...
int CustomTabCtrl::MoveItem(int item_src, int item_dst, bool mouse_sel)
{
	...
	if (ctrl_tool_tip_.m_hWnd)
	{
		for (int i= static_cast<int>(a_items_.GetSize()-1); i>item_dst; i--)
		{
			CString s;
			ctrl_tool_tip_.GetText(s, this, i);
...
LPARAM CustomTabCtrl::GetItemData(int item) const
{
	if (item<0 || item>=a_items_.GetSize())
		return 0;
	return static_cast<LPARAM>(a_items_[item]->param_);
}
...
//	GetClientRect(&cl);
	...
	case RECALC_LAST_PRESSED:
		{
			item_ndx_offset_ = static_cast<int>(a_items_.GetSize()-1);
		}
...
BOOL CustomTabCtrl::PreTranslateMessage(MSG* msg)
{
	...
	return CControlBar::PreTranslateMessage(msg);
}
...
void CustomTabCtrl::OnUpdateCmdUI(CFrameWnd* pTarget, BOOL bDisableIfNoHndler)
{
}

CSize CustomTabCtrl::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
	CControlBar::CalcFixedLayout(bStretch, bHorz);
	return CSize(0x7fff, ideal_height_);
}
