...
public:
	...
	void SCI_METHOD Lex(unsigned int startPos, int length, int initStyle, IDocument* access);
	void SCI_METHOD Fold(unsigned int startPos, int length, int initStyle, IDocument* access);
...
void SCI_METHOD LexerAsm::Lex(unsigned int startPos, int length, int initStyle, IDocument* pAccess)
{
	...
	for (; sc.More(); sc.Forward())
	{
		...
		if (sc.state == SCE_ASM_DEFAULT)
		{
			...
			else if (isascii(sc.ch) && (sc.chNext != 'x' && sc.chNext != 'X') &&
				(isdigit(sc.ch) || (sc.ch == '.' && isascii(sc.chNext) && isdigit(sc.chNext))) ||
				(sc.ch == '$' && (isdigit(sc.chNext) || sc.chNext >= 'a' && sc.chNext <= 'f' || sc.chNext >= 'A' && sc.chNext <= 'Z')))
			{
...
void SCI_METHOD LexerAsm::Fold(unsigned int startPos, int length, int initStyle, IDocument *pAccess) {
	...
	for (unsigned int i = startPos; i < endPos; i++)
	{
		...
		if (options.foldSyntaxBased && (style == SCE_ASM_DIRECTIVE))
		{
			...
			if (styleNext != SCE_ASM_DIRECTIVE)
