...
public:
 ...
 //   STDMETHODIMP_(ULONG) Release (void);
	...
	size_t	FormatLine(BYTE *data, size_t length, size_w offset, size_w skip, TCHAR *buf, size_t buflen, ATTR *attrList, seqchar_info *infobuf, bool fIncSelection);
	...
	void ContentChanged(const unsigned char* data, size_w offset, size_w length, UINT method);
	...
	BOOL InitBuf(const BYTE* buffer, size_w length);
	//BOOL CloseFile();
	//BOOL ClearFile();
	//BOOL RevertFile();
...
private:
	...
	size_t ReadData(size_t offset, seqchar* dest, size_t length);
};	
