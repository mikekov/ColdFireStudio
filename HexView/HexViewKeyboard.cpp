...
bool HexView::ForwardDelete()
{
	...
	if(success)
		ContentChanged(nullptr, m_nCursorOffset, length, HVMETHOD_DELETE);
...
bool HexView::BackDelete()
{
	...
	if(success)
		ContentChanged(nullptr, offset, length, HVMETHOD_DELETE);
...
LRESULT HexView::OnChar(UINT nChar)
{
	...
	if(m_nWhichPane == 0)	// hex column
	{
		...
		else
		{
			...
			BYTE buf= val;
			ContentChanged(&buf, m_nCursorOffset, 1, HVMETHOD_OVERWRITE);
