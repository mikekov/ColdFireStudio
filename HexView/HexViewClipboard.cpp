...
//	m_pDataSeq->group();
	...
	ContentChanged(nullptr, offset, size, HVMETHOD_OVERWRITE);
...
//	}
	// regular data entry
	//ContentChanged(pSource, m_nCursorOffset, nLength, );
	//else
	{
//		if(m_nEditMode == HVMODE_OVERWRITE)
	...
	if(m_fRedrawChanges)
		ContentChanged(pSource, offset, nLength, m_nEditMode == HVMODE_INSERT ? HVMETHOD_INSERT : HVMETHOD_OVERWRITE);
