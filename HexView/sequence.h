...
{
public:
	sequence() : buffer_(nullptr), length_(0), c_(0)
	{}
	...
	size_w size() const { return length_; }
	...
	size_t render(size_w index, seqchar* dest, size_t length, seqchar_info* infobuf= nullptr) const;

	bool replace(size_w index, const seqchar* buf, size_w length);

	bool isreadonly() const { return false; }

	seqchar& getlastmodref(size_w* offset= nullptr) { return c_; }

	bool rendersnapshot(size_t count, span_desc* desclist, size_w offset, seqchar* buf, size_t length) { return false; }

	bool takesnapshot(size_w index, size_w length, span_desc* desclist, size_t* desclen) { return false; }

	typedef std::function<size_t (size_t offset, seqchar* dest, size_t length)> read_fn;

	bool init(/*const seqchar* buffer, size_t length, bool duplicate_buf,*/ size_w length, const read_fn& read);

private:
	const seqchar* buffer_;
	size_w length_;
	seqchar c_;
	read_fn read_;
};
