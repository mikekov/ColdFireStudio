	...
	m_fCursorMoved(true)
{
	...
	m_ColourList[HVC_SELECTION3]  = 0xffb3a7; //COLOR_3DFACE | HEX_SYS_COLOR;
...
void HexView::ContentChanged(const unsigned char* data, size_w offset, size_w length, UINT method)
{
	NMHVCHANGED nmchanged = { { 0,0,0 }, method, offset, length, data };
	UpdateMetrics();
	ScrollToCaret();
...
LRESULT HexView::WndProc(UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case HVM_INITBUF_SHARED:
		return InitBuf((const BYTE *)wParam, lParam ? (size_t)lParam : 1LL << 32);
...
BOOL HexView::InitBuf(const BYTE* buffer, size_w len)
{
	if (m_pDataSeq->init(len, std::bind(&HexView::ReadData, this, _1, _2, _3)))
	{
		m_szFilePath[0] = '\0';
...
size_t HexView::ReadData(size_t offset, seqchar* dest, size_t length)
{
	NMHREADDATA read;
	read.dest_buf = dest;
	read.offset = offset;
	...
	return NotifyParent(HVN_READ_DATA, &read);
}
