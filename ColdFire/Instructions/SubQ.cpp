...
class SubQ : public InstructionImpl<Stencil_REG_EA>
{
public:
	SubQ() : InstructionImpl("SUBQ", IParam().Sizes(IS_LONG).SrcModes(AM_IMMEDIATE).ImmDataRange(1, 8).DestModes(AM_ALL_DST).Opcode(0x5180).Mask(Stencil::MASK))
	{}
	...
	virtual void Execute(Context& ctx) const
	{
		...
		int ext_words= 0;
		auto da= ctx.DecodeMemoryAddress(o.ea_mode, size, ext_words);
...
static Instruction* instr= GetInstructions().Register(new SubQ());
